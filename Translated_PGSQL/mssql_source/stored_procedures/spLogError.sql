CREATE OR REPLACE PROCEDURE public.splogerror(IN _emaillistid integer, IN _mailing integer, IN _externaluniqueid uuid, IN _comment text, OUT _logid integer)
    ...
    BEGIN
        ...
        RAISE NOTICE '- %s - %s', current_timestamp, current_user;

        RAISE NOTICE '    in %s at line %d. Error # %d, state %d, severity %d.', current_user, current_line, current_error, current_state, current_severity;
