...
BEGIN
    ...
    INNER JOIN public.csn_notif_batch_dbo_tblplemaillists AS emaillist ON (bannercontent.storeid = emaillist.storeid)
    INNER JOIN public.dbo_tblplcustomerattributetypes AS customerattributetype ON (bannercontent.customerattributetypeid = customerattributetype.customerattributetypeid)
    INNER JOIN public.dbo_tblcustomerattributestore AS customerattributestore ON (bannercontent.storeid = customerattributestore.storeid
    ...
    INNER JOIN public.dbo_tblplweekday AS weekdays ON (bannercontent.daynumber = weekdays.weekdayid)
;
END;
$function$
;