...
SELECT (
    ...
    + '_'
    + REPLACE(REPLACE(COALESCE(customerattributetype.name, 'MissingCustomerAttributeTypeName'), ' ', ''), 'InMarket', 'IM')
    + '_'
    + REPLACE(REPLACE(COALESCE(customerattributestore.name, 'MissingCustomerAttributeStoreName'), ' ', ''), 'InMarket', 'IM')
    + '_'
...
INNER JOIN dbo.csn_notif_batch_dbo_tblplemaillists AS emaillist ON (bannercontent.storeid = emaillist.storeid)
INNER JOIN dbo.dbo_tblplcustomerattributetypes AS customerattributetype ON (bannercontent.customerattributetypeid = customerattributetype.customerattributetypeid)
INNER JOIN dbo.dbo_tblcustomerattributestore AS customerattributestore ON (
...
INNER JOIN dbo.dbo_tblplweekday AS weekdays ON (bannercontent.daynumber = weekdays.weekdayid)
;
$function$
;